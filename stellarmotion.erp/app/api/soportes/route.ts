...
export async function POST(request: Request) {
  try {
    ...
    const createData = {
      ...
      showApproximateLocation: data.showApproximateLocation ?? false,
      approximateRadius: data.approximateRadius ?? 500,
      priceRangeEnabled: data.priceRangeEnabled ?? false,
      priceMin: data.priceMin ?? null,
      priceMax: data.priceMax ?? null,
      rentalPeriod: data.rentalPeriod ?? 'meses',
