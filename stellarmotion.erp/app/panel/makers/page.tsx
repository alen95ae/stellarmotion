...
export default function MakersPage() {
  ...
  const fetchContacts = async (page = 1) => {
    ...
    try {
      ...
      toast.error("Error al cargar makers");
      setContacts([]);
      setAllContactsForSort([]);
    } finally {
      setLoading(false);
    ...
    (async () => {
      try {
        const [rOrigen, rSector, rInteres] = await Promise.all([
          fetch("/api/contactos/unique-values?field=origen&relation=MAKER").then((r) => r.json()),
          fetch("/api/contactos/unique-values?field=sector&relation=MAKER").then((r) => r.json()),
          fetch("/api/contactos/unique-values?field=interes&relation=MAKER").then((r) => r.json()),
        ]);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("¿Eliminar este maker?")) return;
    try {
      const res = await fetch(`/api/contactos/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error("Error al eliminar");
      toast.success("Maker eliminado");
      fetchContacts(1);
    } catch {
  ...
  function bulkExportSelection() {
    ...
    a.href = url;
    a.download = `makers-seleccion-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  async function bulkDelete() {
    if (selectedIds.length === 0) return;
    if (!confirm(`¿Eliminar ${selectedIds.length} maker(s)?`)) return;
    let ok = 0;
    for (const id of selectedIds) {
      try {
        const res = await fetch(`/api/contactos/${id}`, { method: "DELETE" });
    ...
    toast.success(`${ok} maker(s) eliminado(s)`);
    setSelected({});
    fetchContacts(1);
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      <main className="w-full px-6 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground mb-2">Makers</h1>
        ...
        <div className="bg-card dark:bg-[#141414] border border-border rounded-lg p-4 mb-6 shadow-sm">
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <Button variant="outline" size="sm" asChild className="border-border text-foreground hover:bg-muted hover:text-foreground dark:border-[#404040] dark:text-[#D1D1D1] dark:hover:bg-[#1E1E1E] dark:hover:text-[#FFFFFF]">
              <Link prefetch={false} href="/panel/makers/leads/papelera">
            ...
            <Link prefetch={false} href="/panel/makers/nuevo">
        ...
        <Card className="dark:bg-[#141414]">
          <CardHeader>
            <CardTitle>Makers ({computedPagination.total})</CardTitle>
            <CardDescription>Lista de makers.</CardDescription>
          </CardHeader>
                    ...
                    <TableRow key={c.id}>
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          <Link prefetch={false} href={`/panel/makers/${c.id}`}>
                            <Button
                              variant="outline"
                              size="sm"
                              title="Editar"
