...
export default function OwnersPage() {
    ...
    (async () => {
      try {
        const [rOrigen, rSector, rInteres] = await Promise.all([
          fetch("/api/contactos/unique-values?field=origen&relation=OWNER").then((r) => r.json()),
          fetch("/api/contactos/unique-values?field=sector&relation=OWNER").then((r) => r.json()),
          fetch("/api/contactos/unique-values?field=interes&relation=OWNER").then((r) => r.json()),
        ]);
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      setSelected({});
      fetchContacts(1);
      toast.success(`${count} owner(s) actualizado(s)`);
    } catch {
    ...
    try {
      ...
      while (hasMore) {
        const res = await fetch(
          `/api/contactos?relation=OWNER&page=${page}&limit=${limit}`
        );
        if (!res.ok) throw new Error("Error al cargar");
        const data = await res.json();
      ...
      <main className="w-full px-6 py-8">
        ...
        <Card className="dark:bg-[#141414]">
          <CardHeader>
            ...
            <CardDescription>Lista de owners.</CardDescription>
          </CardHeader>
