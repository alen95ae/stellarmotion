  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) {
          if (res.status === 404) {
            toast.error("Brand no encontrado");
            router.replace("/panel/brands");
            return;
          }
        ...
        setForm({
          kind,
          roles: Array.isArray(data.roles) && data.roles.length > 0 ? data.roles.map((r: string) => r.toLowerCase()) : ["brand"],
        ...
        toast.error("Error al cargar el brand");
        router.replace("/panel/brands");
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="min-h-screen bg-background">
      <main className="w-full px-6 py-8">
        <div className="mb-6">
          <h1 className="text-3xl font-semibold text-foreground">Editar brand</h1>
          <p className="text-muted-foreground text-sm mt-1">Modifica la información del contacto.</p>
        </div>
          <div className="mb-6 flex justify-end gap-2">
            <Link prefetch={false} href="/panel/brands">
              <Button
                type="button"
                variant="outline"
          ...
          <form id="owner-form" ref={formRef} onSubmit={handleSubmit} noValidate className="space-y-6">
            {/* Información básica */}
