...
import BulkActionsPapelera from "@/components/brands/BulkActionsPapelera";
...
export default function BrandsLeadsPapeleraPage() {
  ...
  async function bulkRestore() {
    ...
    if (!confirm(`¿Devolver ${selectedIds.length} registro(s) a la lista de Brands?`)) return;
    try {
      const res = await fetch("/api/leads/restore", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <main className="w-full px-6 py-8">
        <div className="mb-8 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Registros movidos a papelera. Puedes restaurarlos a la lista de Brands.
            </p>
          </div>
          <Button
            variant="outline"
            size="sm"
            onClick={() => router.push("/panel/brands")}
        ...
        <Card className="dark:bg-[#141414] dark:border-[#1E1E1E]">
          <CardHeader>
            ...
            <CardDescription>Registros movidos a papelera. Puedes restaurarlos a la lista de Brands.</CardDescription>
          </CardHeader>
