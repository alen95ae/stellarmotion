...
export default function NuevoOwnerPage() {
  ...
  const fetchPersonaSuggestions = useCallback(async (query: string) => {
    ...
    try {
      ...
      const filtered = data.filter((c) => !form.persona_contacto.some((p) => p.nombre === c.displayName));
  ...
  return (
    <div className="min-h-screen bg-background">
      <main className="w-full px-6 py-8">
        <div className="mb-6">
          <h1 className="text-3xl font-semibold text-foreground">Nuevo brand</h1>
          <p className="text-muted-foreground text-sm mt-1">Añade un nuevo Brand a tu base de datos.</p>
        </div>
