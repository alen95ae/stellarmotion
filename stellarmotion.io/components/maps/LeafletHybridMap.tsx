...
export default function LeafletHybridMap({
  ...
  zoom?: number;
  searchLocation?: { lat: number; lng: number; label?: string };
}) {
  ...
  // Effect to center map when searchLocation changes after map is initialized
  useEffect(() => {
    if (searchLocation && (window as any).mapInstance && (window as any).L) {
      const map = (window as any).mapInstance;
      const L = (window as any).L;
      
      console.log('Centering existing map to search location:', searchLocation);
      
      // Center the map
      map.setView([searchLocation.lat, searchLocation.lng], 15);
      
      // Add a temporary marker for the search location
      const searchMarker = L.marker([searchLocation.lat, searchLocation.lng], {
        ...
        <div style="min-width: 120px;">
          <div style="font-weight: 600; color: #dc2626; font-size: 12px;">
            📍 ${searchLocation.label || 'Ubicación buscada'}
          </div>
        </div>
      `);
  ...
  useEffect(() => {
    ...
    const loadLeaflet = async () => {
      ...
      const initMap = () => {
        ...
        // Check if we have a search location to center on
        if (searchLocation) {
    ...
    // Cargar Leaflet y inicializar el mapa
    loadLeaflet();
  ...
  }, [points, center, zoom, mapId, isClient, searchLocation]);
