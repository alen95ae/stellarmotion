...
export default function OpenStreetSearchBar({ 
  defaultKeywords = "", 
  defaultLocation = "", 
  className = "" 
}: Props) {
  ...
  const fetchSuggestions = async (query: string) => {
    try {
      ...
      // Usar Nominatim API para buscar lugares
      ...
      if (response.ok) {
        ...
        console.error("❌ Error fetching OpenStreetMap suggestions:", response.status);
        setSuggestions([]);
        setShowSuggestions(false);
      }
    } catch (error) {
