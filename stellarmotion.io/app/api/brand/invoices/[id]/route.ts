...
/**
 * GET /api/brand/invoices/[id]
 * Una factura solo si brand_contacto_id = contacto_id del usuario.
 */
export async function GET(
  ...
  try {
    ...
    if (error || !row) {
      ...
      console.error('[brand/invoices] get one error:', error);
      return NextResponse.json(
    ...
    const ownerId = invoice.owner_contacto_id;
    if (ownerId) {
      const { data: c } = await supabase
        ...
        .eq('id', ownerId)
        .single();
      (invoice as Invoice & { owner_name?: string }).owner_name = (c?.razon_social || c?.nombre || '—')?.trim() || '—';
    }

    return NextResponse.json({ invoice });
  } catch (e) {
    console.error('[brand/invoices] GET [id] error:', e);
