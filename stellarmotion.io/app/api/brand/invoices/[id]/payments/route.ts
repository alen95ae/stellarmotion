...
/**
 * GET /api/brand/invoices/[id]/payments
 * Pagos de la factura. Solo si la factura pertenece al brand (brand_contacto_id = contacto_id).
 */
export async function GET(
  ...
  try {
    ...
    if (invError || !invoice) {
      ...
      console.error('[brand/invoices] payments ownership check:', invError);
      return NextResponse.json({ error: 'Factura no encontrada' }, { status: 404 });
    }
    ...
    if (error) {
      console.error('[brand/invoices] payments list error:', error);
      return NextResponse.json(
    ...
    console.error('[brand/invoices] GET payments error:', e);
