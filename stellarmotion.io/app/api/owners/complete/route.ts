...
export async function POST(req: Request) {
  try {
    ...
    console.log("📡 [WEB] Enviando completado de owner al ERP...");

    const ERP_BASE_URL = process.env.NEXT_PUBLIC_ERP_API_URL || process.env.ERP_BASE_URL || 'http://localhost:3000';
    const response = await fetch(`${ERP_BASE_URL}/api/owners/complete`, {
      method: "POST",
      headers: {
    ...
    console.error("🔥 [WEB] Error en complete owner API:", error);
    return NextResponse.json(
